...
var SPEED = 50	# The speed with which the chain moves
var tipmax = 300
...
func shoot(dir: Vector2) -> void:
	direction = (dir*tipmax).normalized()	# Normalize the direction and save it
...
func _physics_process(_delta: float) -> void:
	...
	if tip_loc.length()>tipmax:
		#flying = false
		#hooked = false
		SPEED =0
		
		if tip_loc.length()>tipmax+50:
			flying = false
			hooked = false
	else: SPEED = 30
